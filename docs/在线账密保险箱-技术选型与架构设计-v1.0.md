...
- 用户账密的加密和解密只在浏览器内发生。
- MySQL 不保存用户账密明文、保险箱主密码或可直接解密的恢复密钥。
...
- 登录成功后更换 Session ID。
- 支持后台撤销指定用户全部会话。
- 网站登录会话过期和保险箱自动锁定是两个不同概念。
- 前端不得使用 Pinia 持久化插件保存解密数据、主密码或恢复密钥。
...
```mermaid
flowchart TD
    MP["保险箱主密码"] --> KDF["Argon2id 密钥派生"]
    KDF --> KEK["主密码包装密钥"]
    RK["随机恢复密钥"] --> RKEK["恢复包装密钥"]
    DEK["随机 256 位保险箱数据密钥"] --> W1["主密码包装后的 DEK"]
    DEK --> W2["恢复密钥包装后的 DEK"]
    KEK --> W1
    RKEK --> W2
...
1. 浏览器首次创建保险箱时生成随机 256 位数据密钥 DEK。
2. 使用 Argon2id 从保险箱主密码派生包装密钥 KEK。
3. 用 KEK 加密包装 DEK，服务端只保存包装结果、盐和 KDF 参数。
4. 生成高强度随机恢复密钥，并独立包装同一个 DEK。
5. 每条账密记录使用 DEK 和随机 192 位 nonce 通过 XChaCha20-Poly1305 加密。
6. 记录 ID、所有者标识和载荷版本写入关联数据，防止密文被替换到其他上下文。

网站登录密码不参与 DEK 派生。重置网站登录密码不会解锁保险箱，也不会改变用户的加密数据。
...
- 手机号只记录脱敏值，例如保留末四位。
- 禁止记录密码、确认密码、主密码、恢复密钥、Cookie、Authorization、TOTP 秘钥、账密字段和解密内容。
...
| 浏览器密码学 | libsodium.js、Argon2id、XChaCha20、恢复密钥 | 三类浏览器和目标手机性能可接受 |
