...
- 账密明文不得出现在 `localStorage`、IndexedDB、URL、错误日志和埋点中；标签页 `sessionStorage` 仅允许暂存 DEK（关闭标签即清除），不得存登录密码或账密明文。
...
登录成功后客户端自动用登录密码解包 DEK，并写入标签页 `sessionStorage` 以便刷新恢复；**无独立解锁门禁**。退出登录清除内存与 `sessionStorage` 中的 DEK。服务端 Session 空闲超时 1 小时。
